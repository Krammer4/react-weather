...
function App() {
  ...
  async function getWeatherFromAPI() {
    try {
      const response: any = await axios.get<IResponse>(
        `https://api.openweathermap.org/data/2.5/weather?q=${currentCity}&lang=ru&units=metric&appid=152b7f66c2df2dc3d5b721c565ed70e6`
      );
    ...
    <div className="bg-bg py-5 px-8 min-h-screen flex-col">
      <div className="flex md:justify-between items-center sm:justify-center">
        <div className="flex items-center">
          ...
          <p className="text-white font-black text-4xl pl-3 lg:block sm:hidden">
            KRAMMER WEATHER
          </p>
        </div>
        ...
        <div className="flex items-center">
          <input
            className="text-center md:px-20 xm:px-2 sm:px-0 py-2 rounded-tl-3xl rounded-bl-3xl md:text-lg sm:text-base focus:outline-yellowMain"
          ...
          />
          <button
            className="bg-yellowMain text-black font-extrabold py-2 px-4 rounded-tr-3xl rounded-br-3xl focus:border-yellow active:outline-0 ms:text-lg sm:text-base"
            onClick={setCity}
          >
            Найти
          </button>
      ...
      <Main
        ...
        counter={counterForImg}
        undefiendCity={undefiendCity}
        headerValue={headerValue}
        counterForImg={counterForImg}
      />
    </div>
  );
}
